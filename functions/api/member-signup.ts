...
export const onRequestPost: PagesFunction<Env> = async (ctx) => {
  try {
    ...
    const { name, email, phone, role } = data as Record<string, unknown>;
    ...
    const cleanRole = role && typeof role === "string" ? sanitizeString(role) : null;
    ...
    // Insert record
    await ctx.env.DB.prepare(`
      INSERT INTO member_signups (name, email, phone, role)
      VALUES (?, ?, ?, ?)
    `)
      .bind(cleanName, cleanEmail, cleanPhone, cleanRole)
      .run();
